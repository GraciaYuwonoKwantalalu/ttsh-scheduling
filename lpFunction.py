            ...
            if clear_flag == True and doc in cr_dict:
                for sdates,detailed in cr_dict[doc].items():
                    ...
                    if day == sdateo:
                        ...
                        elif detailed[1] == "Nocallonly":
                            doctor_monthly_activity.append(1)
                        # Doctor specify to not have call (Negative)
                        elif detailed[1] == "Nocall&noweekendduty":
                            doctor_monthly_activity.append(1)
...
def leave_matrix(doc_list,query_start_date,query_last_date,delta):
    ...
    la_dict = readLeaveApplication(doc_list,string_start_date, string_end_date)
...
def run_lp(doctor_call_daily, day_off_monthly, max_call_month_4, max_call_month_5, query_start_date, query_last_date, doc_list, A, B, C, D):
    ...
    combined_call = call_matrix(doc_list,sdate,edate,delta)
    combined_leave = leave_matrix(doc_list,sdate,edate,delta)
    ...
    #Vector to assign priority per day to each doctor. (refer to line 6) wx still kiv cause is hard code
    Requests = request
    ...
    #Fairness Contraint (changed)
    # for i in Doctors:
    #     for j in Days:
    #         prob+= lpSum(vars[i][j]*Points[i][j]) - lpSum(vars[i][j]*Points[i][j])/len(Doctors) <= 3
